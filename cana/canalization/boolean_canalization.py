...
def computes_ts_coverage(k, outputs, two_symbols):
    ...
    # 2**k states (the original did `_ts_covers` per state, which re-expands the
    # schema each time), expand each schema ONCE and mark the states (minterms)
    # it covers directly. Iterating `t` ascending then schemas in list order
    ...
    for t in (0, 1):
        for implicant, permut_indxs, same_symbols_indxs in two_symbols[t]:
            if permut_indxs:
                concrete_implicants = _expand_ts_logic_fast(implicant, permut_indxs)
            else:
