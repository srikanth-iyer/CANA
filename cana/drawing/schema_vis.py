...
def plot_anni_gen_schemata(n, plotTS=True):
    # Init values from BooleanNode
    k = n.k if n.k >= 1 else 1
    outputs = np.array(n.outputs)
    ...
    if not plotTS:
        ...
        tsas = annihilation._two_symbols[1]
        tsgs = generation._two_symbols[1]
        n_ts = sum(len(tss) for tss in [tsas, tsgs])

    pias = annihilation._prime_implicants.get("1", [])
    pigs = generation._prime_implicants.get("1", [])

    # Count number of PI and TS
    n_pi = sum(len(pis) for pis in [pias, pigs])
    # Schemata Cell Width and spacing
    cwidth = 60.0
    cxspace = 0
    cyspace = 6
    ...
    for out, tss in zip([1, 0], [tsgs, tsas]):
        for ts, pss, _ in tss:
            x = 0.0
            xticks = []
            for i, input in enumerate(ts):
