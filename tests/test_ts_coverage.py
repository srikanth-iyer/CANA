...
#
# The reference implementation below reproduces the original per-state
# `computes_ts_coverage` body and relies on `_ts_covers` (the Cython-backed
# coverage test), so equality between the reference and the optimized function
# is a byte-for-byte check.
#
# NOTE: these tests require the `schematodes` package (used by
# `find_two_symbols_v2`) and the compiled `cboolean_canalization` extension.
import math

import pytest

from cana.boolean_node import BooleanNode
import cana.canalization.boolean_canalization as B
from cana.cutils import statenum_to_binstate


def _reference_ts_coverage(k, outputs, two_symbols):
    """Original per-state `computes_ts_coverage`, using `_ts_covers`."""
    ts_coverage = {}
    for statenum in range(2 ** k):
        ...
        for t in output:
            for implicant, permut_indxs, same_symbols_indxs in two_symbols[t]:
                if B._ts_covers(implicant, permut_indxs, binstate):
...
def _coverage_pair(outputs):
    ...
    new = B.computes_ts_coverage(k, node.outputs, two_symbols)
